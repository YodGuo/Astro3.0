import type { APIRoute } from "astro";
import { d1All } from "../../../lib/db";
import { requireAdmin, AuthError } from "../../../lib/auth-guard";
import { getEnv } from "../../../lib/env";
import { clearNotificationSettingsCache } from "../../../lib/notification/settings-cache";
import { apiError, apiUnavailable, apiBadRequest, apiSuccess, apiUnauthorized, apiForbidden } from "../../../lib/api-response";

export const prerender = false;

// Allowed keys for notification settings (prevent arbitrary key writes)
const NOTIFICATION_SETTING_KEYS = new Set([
  "sse_enabled",
  "toast_preferences",
  "rate_limit",
  // Template keys follow the pattern "template:<eventType>"
]);

// GET /api/notifications/settings — Get all notification settings
export const GET: APIRoute = async ({ locals }) => {
  try {
    requireAdmin(locals);
  } catch (e) {
    if (e instanceof AuthError) {
      if (e.status === 401) return apiUnauthorized(e.message);
      return apiForbidden(e.message);
    }
    throw e;
  }

  const env = await getEnv();
  const d1 = env.DB;
  if (!d1) return apiUnavailable("DB not available");

  try {
    const result = await d1All<{ key: string; value: string }>(d1, "SELECT key, value FROM notification_settings");

    const settings: Record<string, unknown> = {};
    for (const row of result) {
      try {
        settings[row.key] = JSON.parse(row.value);
      } catch {
        settings[row.key] = row.value;
      }
    }

    return apiSuccess(settings);
  } catch (e) {
    return apiError(e);
  }
};

// PUT /api/notifications/settings — Update notification settings
export const PUT: APIRoute = async ({ request, locals }) => {
  requireAdmin(locals);
  const env = await getEnv();
  const d1 = env.DB;
  if (!d1) return apiUnavailable("DB not available");

  try {
    const body = await request.json() as { key: string; value: unknown };

    if (!body.key || typeof body.key !== "string") {
      return apiBadRequest("key is required");
    }

    // Whitelist check: allow known keys or template:* pattern
    if (!NOTIFICATION_SETTING_KEYS.has(body.key) && !body.key.startsWith("template:")) {
      return apiBadRequest(`Invalid notification setting key: ${body.key}`);
    }

    // Validate value type and length
    if (body.value === undefined || body.value === null) {
      return apiBadRequest("value is required");
    }
    const valueStr = typeof body.value === "string" ? body.value : JSON.stringify(body.value);
    if (valueStr.length > 50000) {
      return apiBadRequest("value too large (max 50000 characters)");
    }

    const value = typeof body.value === "string" ? body.value : JSON.stringify(body.value);

    await d1
      .prepare(
        `INSERT INTO notification_settings (key, value, updated_at) VALUES (?, ?, unixepoch())
         ON CONFLICT(key) DO UPDATE SET value = excluded.value, updated_at = unixepoch()`
      )
      .bind(body.key, value)
      .run();

    // Clear notification settings cache
    await clearNotificationSettingsCache({ db: d1, env });

    return apiSuccess({ success: true, key: body.key });
  } catch (e) {
    return apiError(e);
  }
};
