...
const users = sqliteTable("users", {
  ...
  // "admin" | "user" (CHECK enforced in DB via migration 0008)
...
const solutions = sqliteTable("solutions", {
  id: text("id").primaryKey(),
  slug: text("slug").notNull().unique(),
...
const quoteRequests = sqliteTable("quote_requests", {
  id: text("id").primaryKey(),
  ...
  // new, contacted, quoted, closed (CHECK enforced in DB via migration 0008)
  createdAt: integer("created_at", { mode: "timestamp" }).notNull().default(sql`(unixepoch())`)
});
const newsPosts = sqliteTable("news_posts", {
  id: text("id").primaryKey(),
  slug: text("slug").notNull().unique(),
  title: text("title").notNull(),
  summary: text("summary"),
  ...
  // draft, published (CHECK enforced in DB via migration 0008)
...
const newsPostTags = sqliteTable(
  "news_post_tags",
  {
...
const comments = sqliteTable("comments", {
  id: text("id").primaryKey(),
  postId: text("post_id").notNull().references(() => newsPosts.id),
  parentId: text("parent_id").references(() => comments.id),
  authorName: text("author_name").notNull(),
  authorEmail: text("author_email").notNull(),
  content: text("content").notNull(),
  status: text("status").default("pending"),
  // pending, approved, rejected (CHECK enforced in DB via migration 0008)
  createdAt: integer("created_at", { mode: "timestamp" }).notNull().default(sql`(unixepoch())`)
});
...
const newsPostsRelations = relations(newsPosts, ({ many }) => ({
...
const newsPostTagsRelations = relations(newsPostTags, ({ one }) => ({
  post: one(newsPosts, {
...
const notificationChannels = sqliteTable("notification_channels", {
  id: text("id").primaryKey(),
  name: text("name").notNull(),
...
const notificationSubscriptions = sqliteTable("notification_subscriptions", {
  id: text("id").primaryKey(),
  channelId: text("channel_id").notNull().references(() => notificationChannels.id, { onDelete: "cascade" }),
  eventType: text("event_type").notNull(),
  createdAt: integer("created_at", { mode: "timestamp" }).notNull().default(sql`(unixepoch())`)
});
const notificationLogs = sqliteTable("notification_logs", {
  id: text("id").primaryKey(),
  eventType: text("event_type").notNull(),
  ...
  eventData: text("event_data", { mode: "json" }).$type(),
  createdAt: integer("created_at", { mode: "timestamp" }).notNull().default(sql`(unixepoch())`)
});
...
const pageSections = sqliteTable("page_sections", {
  id: text("id").primaryKey(),
...
const notificationSettings = sqliteTable("notification_settings", {
  key: text("key").primaryKey(),
  value: text("value").notNull(),
  // JSON string
  updatedAt: integer("updated_at", { mode: "timestamp" }).notNull().default(sql`(unixepoch())`)
});
const notificationChannelsRelations = relations(notificationChannels, ({ many }) => ({
  subscriptions: many(notificationSubscriptions)
}));
const notificationSubscriptionsRelations = relations(notificationSubscriptions, ({ one }) => ({
...
const schema = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  ...
  media,
  newsPostTags,
  newsPostTagsRelations,
  newsPosts,
  newsPostsRelations,
  notificationChannels,
  notificationChannelsRelations,
  notificationLogs,
...
async function d1All(d1, query, ...bindings) {
  const stmt = bindings.length > 0 ? d1.prepare(query).bind(...bindings) : d1.prepare(query);
